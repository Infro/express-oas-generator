...
module.exports.processHeaders = (req, method, spec) => {
  ...
  var add_security_skip = true;
  headers
    .filter(h => h.toLowerCase() === 'authorization')
    .forEach(h => {
      add_security_skip = false;
...
function updateResponses(res, method, chunks) {
  ...
  try {
    ...
    schema = method.responses[res.statusCode]
      ? utils.mergeSchemas([method.responses[res.statusCode].schema ?? {}, schema])
      : schema;
    // schema = mergeSchemas(method.responses[res.statusCode]?.schema ?? {}, schema);
  } catch (ex) {
    const type = utils.getType(body);
    schema = method.responses[res.statusCode].schema ||{
      type: type,
